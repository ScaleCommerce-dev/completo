...
describe('POST /api/boards/:id/columns', async () => {
  ...
  it('creates a status and links only to the requesting board', async () => {
    ...
    const newColIn1 = full1After.columns.find((c: Record<string, unknown>) => c.name === 'Custom Status')
    const newColIn2 = full2After.columns.find((c: Record<string, unknown>) => c.name === 'Custom Status')
