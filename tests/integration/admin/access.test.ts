...
describe('Admin access', async () => {
  ...
  let _card: { id: number, statusId: string, projectId: string }
  ...
  it('admin can list all projects including non-member ones', async () => {
    ...
    const found = projects.find((p: Record<string, unknown>) => p.id === project.id)
  ...
  it('admin can view a board on a non-member project', async () => {
    ...
    expect((res.columns as unknown[]).length).toBeGreaterThan(0)
  })
  ...
  it('admin can manage members (owner-level action) on non-member projects', async () => {
    ...
    const found = members.find((m: Record<string, unknown>) => m.id === extra.id)
    expect(found).toBeDefined()
  ...
  it('admin my-tasks only returns their own assigned cards', async () => {
    ...
    const projectGroup = (data.groups as Record<string, unknown>[]).find((g: Record<string, unknown>) => (g.project as Record<string, unknown>)?.id === project.id)
