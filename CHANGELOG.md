...
- **A board card reads as a title.** The first thing on every card used to be its ticket number, with the title on line two and the description underneath — so a card carrying a spec rendered as a paragraph, and a column of twenty read as a wall of identifiers. Titles come first and are capped at two lines, tags are a coloured dot and a name instead of a filled uppercase pill, and the ticket ID sits on a quiet footer line where it is still one click to copy. Priority is a thin coloured edge, and only High and Urgent get one, so an urgent card actually stands out. Cards fit their content instead of always reserving a footer row, assignees show their picture rather than initials, and the "N/A" badge on unassigned cards is gone.
- **Boards choose whether cards show a description.** Two lines of it sit under the title, quiet enough to read as background rather than competing with the title — and a switch under view settings → Display turns them off for a board where they don't help. On by default; boards that turn it off go back to a small glyph marking the cards that have one.
