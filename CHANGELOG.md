...
- **Boards choose what their cards show.** View settings → Display has a switch for the description excerpt and one for the tag row, both on by default, because whether either helps depends entirely on how a team writes cards. The description is two lines under the title, quiet enough to read as background; turn it off and a small glyph still marks the cards that have one. Turn tags off and they are still one click away on the card itself.
