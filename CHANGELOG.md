...
- **Walk a board from inside a card.** With a card open, two chevrons beside the close button step to the previous or next card in the same column, and `↑` `↓` do the same from the keyboard — the tooltips say so. `←` and `→` jump to the top of the column either side, with the board sliding across to follow. Triaging a column, or reading through everything In Progress before a standup, no longer means closing and reopening a card for each one. Arrow keys still belong to whatever you are actually using: a description or comment you are typing in, an open status or priority menu, the date picker. Empty columns are skipped, and the ends of a column and of the board stop rather than wrapping around.
