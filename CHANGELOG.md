...
- **You can add a card by typing.** "New card" opens a box in the column instead of the full card dialog, and stays open so you can type a backlog in one go. A column header carries one control, `+` — the `…` menu beside it only offered that same action a click further away, plus the board's own settings dialog, which is already in the view header.
