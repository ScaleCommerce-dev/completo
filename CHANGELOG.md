...
- **Cards open in a side panel.** Clicking a card slides a panel in from the right instead of covering the board with a centred dialog, so you can still see where the card sits. The five property rows are now one line of chips pinned below the title, so status and assignee stay readable while you scroll the comments, and the description and comments are visible the moment the panel opens. On a new card, typing a title and pressing Enter creates it. The card's own page shows the same controls, with its timestamps labelled and no empty attachment block pushing the comments off-screen.
- **The card panel's toolbar is its own row.** Moving through the board, opening the card as a page, the card's own menu and closing it now sit together on one line above the ticket ID, instead of sharing it. Close is set apart from the rest, so it no longer reads as a fourth icon in a row of four, and on a phone the controls and the author's name have stopped competing for the same 350 pixels. Status, assignee, priority, due date and tags share one line and wrap only when they run out of room — tags no longer take a line of their own on every card, and where the line does break they carry on beside whatever came before them.
- **Walk a board from inside a card.** With a card open, four controls beside the close button step through it: up and down for the previous and next card in the same column, left and right for the top of the column either side, with the board sliding across to follow. Each one names its keyboard shortcut when you point at it, so `↑` `↓` `←` `→` do the same thing without reaching for the mouse, and a direction with nowhere to go is greyed out. Triaging a column, or reading through everything In Progress before a standup, no longer means closing and reopening a card for each one. Arrow keys still belong to whatever you are actually using: a description or comment you are typing in, an open status or priority menu, the date picker. Empty columns are skipped, and the ends of a column and of the board stop rather than wrapping around.
...
- **Small things on the card panel.** Its three section headings — description, attachments, comments — were set at three different letter-spacings, and only two of them had icons. Content scrolling past the pinned header fades out instead of being sliced through the middle of a line of text. Comment edit and delete buttons stay out of the way until you reach the comment, matching attachments. Opening a card no longer puts the keyboard on "open as a full page", where pressing Enter left the board you had just opened the card from. And the `⌘↵` hint now rides on the button it triggers everywhere it appears, rather than floating next to it in some places and sitting inside it in others.
- **A card with nothing on it looks like a card with nothing on it.** Opening an empty card used to show three headings, two of them with nothing underneath, and a full markdown editor — tabs, a formatting toolbar, an AI button — sitting open and empty for a comment nobody was writing, directly below an identical one for the description. The comment box is now a single line saying "Leave a comment…" that opens when you click it, and an empty description offers "Add a description…" where the blank space was. Each empty section says what goes there instead of announcing that it is empty. The comment box also no longer asks you to "describe the task".
- **Save sits under the thing it saves.** A card's description is committed by a Save button directly beneath its editor, with `⌘↵` on it, and Cancel beside it — instead of a button at the far end of the panel that you reached past the attachments and every comment, and which sat greyed out on every card you were merely reading. Clicking the description itself starts editing it, so changing a paragraph no longer means finding a small "Edit" first; links and text selection still behave as text. Everything else on a card already saved itself, so the card panel has no button bar at all now and the card's own page has no Save in its sidebar — the height goes to the card. Delete moved to a `⋯` menu beside the close button, and both card surfaces now ask about deleting the same way the rest of the app does.
...
- **Tag colours are readable in dark mode, and tags keep the name you gave them.** A dark tag colour used to sit on a faint wash of itself and effectively disappear, and the same tag rendered at two different tints depending on the screen. Tag, status and project colours are now adjusted for whichever theme you are in. A tag you named "ux/ui" also no longer displays as "UX/UI".
