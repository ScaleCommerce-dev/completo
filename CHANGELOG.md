...
### App
- **A board card reads as a title now.** The first thing on every card used to be its ticket number, with the title on line two and the description underneath — so a card carrying a spec rendered as a paragraph, and a column of twenty read as a wall of identifiers. Titles come first and are capped at two lines, tags are a coloured dot and a name instead of a filled uppercase pill, and the ticket ID has moved to a quiet footer line where it is still one click to copy. Cards are shorter as a result.
- **You can set a due date and change tags from a board card.** Both used to appear only when they already had a value, so the one card that needed a date was the one card that couldn't be given one without opening it. Every card now has the same four controls in the same four places — tags, priority, due date, assignee — each showing its value when set and appearing on hover when not. They're all the same size, all say what they are on hover, and all light up the same way when you point at one; the assignee used to be outlined while the rest filled. Nothing extra shows on a card at rest.
- **Status, priority, assignee and tags all open the same menu now.** They are the same act — choose from a list, the chosen ones are marked — but they had drifted into six implementations across the board, the card and the list, at three different widths and with three different ways of marking the selection. Every one of them now names the field at the top, puts its dot, icon or face on one left edge, and marks the current value the same way, whether you may pick one value or several. They all gained arrow-key navigation and type-to-jump; tag and status colours line up with their labels instead of floating above them; the status menu on the card finally shows the status colours at all. The date picker stays a calendar, which is the one field that genuinely isn't a list.
- **My Tasks can change a card's status and tags.** It has always been the same table as a project's list view, but two of its columns were read-only, so the one screen built for working through your own queue was the one screen where you could not move a card to In Progress.
- **Tag names are readable in dark mode in the tag picker.** It was printing each name in the tag's own colour, so a dark tag on a dark menu was all but invisible — the last place in the app still doing that. It also stopped forcing tag names into capitals.
- **Fixed: clicking a card's priority or assignee made every icon on the card vanish.** They came back as soon as you moved the mouse, but the menu you had just opened appeared to have nothing behind it.
- **Fixed: closing a card's field menu with Escape left its tooltip on screen** until the next click, and left a focus ring around an icon you had only clicked. Dismissing a menu with Escape now leaves things exactly as clicking elsewhere does. Opening one with the keyboard still hands focus back, so tabbing continues from where you were.
- **A card's controls are all in one place.** The two copy buttons that slid out of the ticket ID sat at the opposite end of the card from everything else, and landed on top of the attachment count while they were there. They're gone from the board: clicking the ticket ID still copies its link, and copying the bare ID is still there on the card itself. "Open in full page" moved up beside the title, since it's the one control that leaves the board rather than changing the card. What's left reads as two halves — the left tells you about the card, the right changes it.
- **Boards choose whether cards show a description.** Two lines of it sit under the title, smaller and quieter than before so they read as background rather than competing with the title — and there is a switch under view settings → Display to turn them off for a board where they don't help. On by default. Cards without a description are unaffected, and boards that turn it off go back to a small glyph marking the cards that have one.
...
- **The sidebar's collapse control stays on the divider.** Collapsed, it sat inside the rail on top of the logo instead of on the edge where you left it.
