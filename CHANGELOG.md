...
- **You can set a due date and change tags from a board card.** Both used to appear only when they already had a value, so the one card that needed a date was the one card that couldn't be given one without opening it. Every card now has the same four controls in the same four places — tags, priority, due date, assignee — each showing its value when set and appearing on hover when not, each saying what it is when you point at it. "Open in full page" moved up beside the title, since it's the one control that leaves the board rather than changing the card. Nothing extra shows on a card at rest.
