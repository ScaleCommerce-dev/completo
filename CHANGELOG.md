...
- **Fixed: list views could render every title blank.** With enough columns showing, the fixed-width ones took all the space and the Title column collapsed to a few pixels — its header overlapping its neighbour and not one row showing a title. It degraded with the width of your window, so it looked fine on a large screen. Title now has a floor and the table scrolls sideways instead. The same table was clipped at both edges, with the ID column hidden behind the sidebar divider and the assignee column running off the right.
