  ...
  <div class="mt-6 rounded-xl border border-default bg-default shadow-sm overflow-hidden">
    <div class="px-5 pt-4 pb-2 flex items-center gap-2">
      <UIcon
        ...
        class="text-[14px] text-dimmed"
      />
      <span class="text-[11px] font-semibold uppercase tracking-[0.08em] text-dimmed">API Tokens</span>
    </div>

    <div class="px-5 pb-3">
      <p class="text-[13px] text-dimmed leading-relaxed">
    ...
    <div class="mx-5 rounded-lg border border-default overflow-hidden">
      <div class="flex items-center gap-2 px-3 py-2.5 bg-default">
        ...
        <select
          v-model="tokenExpiry"
          class="text-[13px] text-muted bg-transparent border-0 outline-none! ring-0! cursor-pointer"
        >
          <option value="">
            No expiry
          </option>
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <code class="flex-1 text-[12px] font-mono text-default bg-default rounded px-2 py-1.5 border border-accented truncate select-all">{{ createdToken }}</code>
        <button
          type="button"
          class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[12px] font-semibold transition-all shrink-0"
          :class="tokenCopied ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30' : 'text-muted hover:bg-elevated'"
          @click="copyToken"
        >
          <UIcon
        ...
        <button
          type="button"
          class="text-[12px] font-medium text-dimmed hover:text-toned hover:text-default transition-colors"
    ...
    >
      ...
      >
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <span class="text-[13px] font-medium text-default truncate">{{ token.name }}</span>
            <code class="text-[11px] font-mono text-dimmed bg-elevated rounded px-1.5 py-0.5 shrink-0">{{ token.tokenPrefix }}...</code>
            <span
          ...
          <div class="flex items-center gap-3 mt-0.5">
            <span class="text-[11px] text-dimmed">Created {{ formatTokenDate(token.createdAt) }}</span>
            <span
        ...
        <div class="shrink-0 ml-3">
          ...
          >
            <button
              type="button"
              class="flex items-center gap-1 px-2 py-1 rounded-md text-[12px] font-medium text-dimmed hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-all opacity-0 sm:group-hover:opacity-100 max-sm:opacity-60"
              style="opacity: 0.6"
          ...
          >
            ...
            <button
              type="button"
              class="px-2 py-1 rounded-md text-[12px] font-medium text-dimmed hover:text-toned transition-colors"
