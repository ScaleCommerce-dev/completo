...
const emit = defineEmits<{
  ...
  update: [cardId: number, updates: any, tagIds: string[]]
...
watch(() => props.card, (card) => {
  if (card) {
    ...
    selectedTagIds.value = (card.tags || []).map((t: any) => t.id)
  ...
  >
    <template #body>
      <form class="flex flex-col" @submit.prevent="submit">
        <!-- Card ID (edit only) -->
        <div v-if="isEdit" class="flex items-center justify-between px-5 pt-5 pb-2">
        ...
        <div class="mx-5 mt-3 grid grid-cols-2 gap-2">
          ...
          <div class="flex items-center gap-2 rounded-lg border border-zinc-200 dark:border-zinc-700/60 bg-white dark:bg-zinc-800/50 px-2.5 py-1.5">
            <span class="w-2 h-2 rounded-full shrink-0 transition-colors" :style="{ backgroundColor: selectedStatusColor }" />
            <USelect
          ...
          <!-- Assignee -->
          <div class="flex items-center gap-2 rounded-lg border border-zinc-200 dark:border-zinc-700/60 bg-white dark:bg-zinc-800/50 px-2.5 py-1.5">
            <UIcon name="i-lucide-user" class="text-[12px] text-zinc-400 shrink-0" />
            <USelect
          ...
          <!-- Due Date -->
          <div class="flex items-center gap-2 rounded-lg border border-zinc-200 dark:border-zinc-700/60 bg-white dark:bg-zinc-800/50 px-2.5 py-1.5">
            <UIcon name="i-lucide-calendar" class="text-[12px] text-zinc-400 shrink-0" />
            <DueDatePicker
        ...
        <!-- Tags -->
        <div v-if="tags?.length" class="mx-5 mt-3">
          <div class="flex items-center gap-1.5 mb-2">
            <UIcon name="i-lucide-tag" class="text-[12px] text-zinc-400" />
          ...
          <UPopover :content="{ align: 'start', side: 'bottom', sideOffset: 4 }">
            ...
            <template #content>
              <TagToggleList :tags="tags" :selected-ids="selectedTagIds" @toggle="toggleTag" />
            </template>
          </UPopover>
        </div>

        <!-- Attachments (edit mode only) -->
        <div v-if="isEdit" class="mx-5 mt-3">
          <AttachmentList :card-id="card?.id" />
        </div>

        <!-- Delete confirmation (edit mode only) -->
        <div v-if="isEdit && showDeleteConfirm" class="mx-5 mt-3 rounded-lg border border-red-200/60 dark:border-red-800/40 bg-red-50/50 dark:bg-red-950/20 p-3 flex flex-col gap-2">
        ...
        <div class="flex items-center justify-end gap-2 px-5 pt-4 pb-5 mt-4 border-t border-zinc-100 dark:border-zinc-700/40">
          ...
          <button
            ...
            :disabled="!title.trim()"
          >
            <UIcon v-if="!isEdit" name="i-lucide-plus" class="text-[14px]" />
            {{ isEdit ? 'Save' : 'Create' }}
          </button>
        </div>
      </form>
