...
function handleCardClick(card: any, projectSlug: string, projectKey: string) {
  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex items-center justify-between px-5 py-2.5 border-b border-zinc-200/80 dark:border-zinc-700/50 bg-white/50 dark:bg-zinc-900/50 backdrop-blur-sm">
      <div class="flex items-center gap-2.5">
        <h1 class="text-[15px] font-extrabold tracking-[-0.02em] text-zinc-900 dark:text-zinc-100">My Tasks</h1>
    ...
    <div class="flex-1 overflow-auto p-4 flex flex-col gap-4">
      <div v-for="group in groups" :key="group.project.id">
        ...
        >
          ...
          <div class="flex items-center justify-center w-5 h-5 rounded bg-zinc-100 dark:bg-zinc-800">
            <UIcon :name="`i-lucide-${group.project.icon || 'folder'}`" class="text-[10px] text-zinc-400" />
          </div>
      ...
      <!-- Empty state -->
      <div v-if="!groups.length" class="text-center py-20">
        ...
        <p class="font-bold text-zinc-900 dark:text-zinc-100">All clear!</p>
        <p class="text-[14px] text-zinc-500 dark:text-zinc-400 mt-1">No tasks assigned to you.</p>
      </div>
    </div>
