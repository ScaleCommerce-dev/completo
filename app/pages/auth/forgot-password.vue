...
<template>
  <!-- Success state -->
  <div v-if="submitted" class="auth-glass">
  ...
  <div v-else class="auth-glass">
    <div class="px-7 pt-7 pb-2">
      <h2 class="text-base font-bold text-center tracking-[-0.02em]">Reset your password</h2>
      <p class="text-[13px] text-center text-zinc-500 dark:text-zinc-400 mt-1">
        Enter your email and we'll send you a reset link
      </p>
    </div>

    <form class="px-7 pb-7 flex flex-col gap-5" @submit.prevent="requestReset">
      ...
      <div class="auth-field" style="animation-delay: 0.1s">
        <UButton type="submit" label="Send Reset Link" block :loading="loading" size="lg" />
      </div>
    </form>
