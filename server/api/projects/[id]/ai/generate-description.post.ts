...
export default defineEventHandler(async (event) => {
  ...
  return sendAiStream(event, stream, promptSource)
})

function buildSystemPrompt(projectBriefing: string): string {
  const prompt = `You are a project management assistant helping write card descriptions for a Kanban board app called Completo. Write clear, actionable descriptions in Markdown format.
