...
**The test only rejects `zinc` and `slate`, so raw `red-*` / `indigo-*` walks past it too.** `ProfileDangerZone` is the worst offender left — `bg-red-50/50 dark:bg-red-950/20`, `border-red-200 dark:border-red-800/50`, `placeholder-zinc-300`, and three hand-rolled buttons where `UButton` and `UiConfirmDialog` exist. Don't copy it as the "danger zone" idiom; it predates all of this.
...
- **`--ui-border`, `--ui-bg-muted` and `--ui-bg-elevated` all resolve to the same 27.4% lightness in dark mode.** A hairline drawn with `--ui-border` around a `--ui-bg-muted` surface draws itself in the surface's own colour and disappears. Use `--ui-border-accented` (37%) where a border must survive on a recessed surface.
...
**User-chosen colours go through `.swatch`.** Tag, status and project-accent hex is user data that has to work on white and on near-black. Set `--swatch` inline and `.swatch` / `.swatch-outline` / `.swatch-dot` / `.swatch-text` / `.swatch-bar` derive a readable foreground, fill and ring per theme via `color-mix()` — no colour-mode logic in JS. Applying the hex raw was the app's worst contrast bug: a dark tag was unreadable in dark mode, and the recipe had been copy-pasted with drifted alphas so the same tag rendered at two tints on two screens.
...
- **Ticket IDs:** `{projectKey}-{cardId}`. On a board card it sits on the footer line with the comment and attachment counts, not on the title line — the number was the first thing the eye hit on every card and nobody was looking for it.
...
- **`UiConfirmDialog`**, **`UiSaveBar`** (footer order is fixed: destructive far left, primary last), **`UiPerson`**, **`UiSectionLabel`**, **`UiStatusDot`**, **`UiFieldRow`**, **`UiDraftNotice`**.
  ...
  - **A comment thread has no lines and no boxes in it — the avatars are the structure.** Two wrong answers came first and both are recorded in the component, because both are tempting:
    - **Horizontal hairlines between comments.** Consistent with the app and still wrong: a divided stack is this app's vocabulary for a *table of uniform fields* (the rail, the attachments list), and banding prose of wildly varying length reads as a spreadsheet. A full-width rule also cuts straight across the avatar gutter, slicing the column the avatars are supposed to own.
    - **Then a vertical connector down that gutter,** GitHub's stub generalised. Its length is whatever the comment above happens to be tall, so between two one-liners it is a 20px tick and below one carrying a code block a 115px rail — it reads as a fragment. A connector earns its keep when the nodes it joins are cards of their own (GitHub) or uniform rows (an activity log); here it was decoration justifying a gutter that needs none.
    - What works is Linear's: a column of avatars, 24px between comments, and **a byline deliberately smaller than the body it introduces** — 12px semibold name over 14px prose. The original defect was not the gap ratio (8:1 was plenty) but that a 13px semibold name and the 14px sentence under it *looked alike*, so four comments read as eight interchangeable lines. Also note the row actions are `absolute`: in the byline's flex row those 20px buttons set its height, so the byline was 20px tall whether or not anything was in it and the body sat 24px below its own author's name — the coupling the whole design rests on, undone by a control that only shows on hover.
